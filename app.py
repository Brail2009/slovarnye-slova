...
# Настройка серверной сессии (хранение в файлах)
app.config['SESSION_TYPE'] = 'filesystem'
app.config['SESSION_FILE_DIR'] = './flask_session'  # папка для файлов сессий
...
def normalize(word):
    ...
    # Секретный ключ – обязательно через переменную окружения
    app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'dev-key-change-in-production')

    # Настройка серверной сессии
    app.config['SESSION_TYPE'] = 'filesystem'
    app.config['SESSION_FILE_DIR'] = './flask_session'  # можно использовать '/tmp/flask_session' для Render
    app.config['SESSION_PERMANENT'] = False
    app.config['SESSION_USE_SIGNER'] = True
    # Увеличиваем таймаут бездействия (опционально)
    app.config['PERMANENT_SESSION_LIFETIME'] = 3600  # 1 час

    # Инициализируем Session после конфига
    Session(app)
